...
- For STRY0010375 Position work in Vår Energi sandbox, use the existing ESM Integrations Framework table `sn_hr_integr_fw_position_staging` and transform map `Position Transform map` (`ff402e54eb943010aa60bc58495228f6`) to target `sn_hr_core_position`. The staging table has scoped caller restrictions, so write staging rows from a public ESM Script Include, then run `GlideImportSetTransformerWorker` from the Global Scripted REST resource; the transformer API is blocked inside scoped applications. The SuccessFactors `department` payload value maps to `cmn_department.id`, not `correlation_id`, and `effectiveStatus=A/I` can be staged in a custom `u_effective_status` column and mapped to `sn_hr_core_position.active` with a scripted field map.
...
- For STRY0010375 HR Profile/User work, do not implement from sheet 5 alone. Clarify the full payload, identity/upsert key, target record split between `sys_user` and `sn_hr_core_profile`, create-vs-update behavior, reference lookup rules for manager/department/position/location, allowed choice values, and termination/status behavior before adding fields, transform logic, or Scripted REST resources.
